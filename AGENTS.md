...
- When a compatible local Linux builder is available, prefer `nix build .#nixosConfigurations.<host>.config.system.build.toplevel --no-link` for pass/fail testing. If the current machine cannot build the target locally, target the remote Linux store directly: `XDG_CACHE_HOME=/tmp/codex-nixfiles-build-cache nix build --eval-store auto --store ssh-ng://joseph@anacreon .#nixosConfigurations.<host>.config.system.build.toplevel --no-link`. A normal distributed build imports missing remote outputs into the local store; direct remote-store outputs are unrooted and may be removed by garbage collection.
