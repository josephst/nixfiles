...
- When a compatible local Linux builder is available, prefer `nix build .#nixosConfigurations.<host>.config.system.build.toplevel --no-link` for pass/fail testing. If the current machine cannot build the target locally, target that host's remote Linux store directly with `--eval-store auto --store ssh-ng://joseph@<host> --no-link`. A normal distributed build imports missing remote outputs into the local store; direct remote-store outputs are unrooted and may be removed by garbage collection.
